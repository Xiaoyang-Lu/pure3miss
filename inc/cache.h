...
class CACHE : public MEMORY {
  public:
    ...
    uint64_t pf_requested,
	     ...
	     pf_late,
             pf_fill;
    uint64_t pure3_miss, pure2_miss;
    // queues
...
//    std::map<uint32_t, set<uint64_t>> L2C_active_hit_cycles_per_core;
	...
	{
/*                
		if(L1D_active_hit_cycles_per_core.find(i) == L1D_active_hit_cycles_per_core.end())
                {
                        L1D_active_hit_cycles_per_core[i] = set<uint64_t>();
                }
                if(L2C_active_hit_cycles_per_core.find(i) == L2C_active_hit_cycles_per_core.end())
                {
                        L2C_active_hit_cycles_per_core[i] = set<uint64_t>();
                }
*/
