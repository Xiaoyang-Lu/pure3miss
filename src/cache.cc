...
std::map<uint32_t, set<uint64_t>> L1D_active_hit_cycles_per_core;
std::map<uint32_t, set<uint64_t>> L2C_active_hit_cycles_per_core;

uint64_t l1_latency = 4;
uint64_t l2_latency = 10;
uint64_t l3_latency = 20;
...
/*
                ...
                { cout << "L1 latency" << current_miss_latency << endl;}
                if(cache_type == IS_L2C)
                { cout << "L2 latency" << current_miss_latency << endl;}
                if(cache_type == IS_LLC)
                { cout << "L3 latency" << current_miss_latency << endl;}
*/
		...
		if(overlap_miss_tag == 1)
		{
			sim_overlap_miss[fill_cpu][MSHR.entry[mshr_index].type]++;
                ...
                if (pure_miss_tag == 1)
                {
                	...
                	if (cache_type == IS_LLC)
                	{
				uint64_t l3_enqueued = MSHR.entry[mshr_index].cycle_enqueued;
				uint64_t l2_enqueued = l3_enqueued - l3_latency;
				uint64_t l1_enqueued = l2_enqueued - l2_latency;

				uint64_t l3_dequeued = current_core_cycle[fill_cpu];
				uint64_t l2_dequeued = l3_dequeued;
				uint64_t l1_dequeued = l2_dequeued;

				//uint64_t l2_dequeued = l3_dequeued + l2_latency;
				//uint64_t l1_dequeued = l2_dequeued + l1_latency;
                                
				//cout << L1D_active_hit_cycles_per_core[fill_cpu].size() << endl;
                                //cout << L2C_active_hit_cycles_per_core[fill_cpu].size() << endl;
				//cout << "l1_enqueued cycle " << l1_enqueued << endl;
				//cout << "l1_dequeued cycle " << l1_dequeued << endl;
				uint64_t l1_pure_miss = 0;
				uint64_t l2_pure_miss = 0;
				for (uint64_t now  = l1_enqueued; now <= l1_dequeued; now++)
				{
					if(L1D_active_hit_cycles_per_core[fill_cpu].find(now) == L1D_active_hit_cycles_per_core[fill_cpu].end())
					{
						l1_pure_miss = 1;
						break;
					}		
				}
                                for (uint64_t now  = l2_enqueued; now <= l2_dequeued; now++)
                                {
                                        if(L2C_active_hit_cycles_per_core[fill_cpu].find(now) == L2C_active_hit_cycles_per_core[fill_cpu].end())
                                        {
                                                l2_pure_miss = 1;
						break;
                                        }
                                }				
				if(l1_pure_miss == 1 && l2_pure_miss == 1)	
				{
					pure3_miss++;
				}
                	}                
...
*/
		...
		sim_overlap_miss[fill_cpu][MSHR.entry[mshr_index].type]++;
            ...
            // pure miss
				...
				//cout << L1D_active_hit_cycles_per_core[fill_cpu].size() << endl;
				//cout << L2C_active_hit_cycles_per_core[fill_cpu].size() << endl;
                                //cout << "l3_enqueued cycle " << l3_enqueued << endl;
...
void CACHE::handle_writeback()
{
    ...
    if ((WQ.entry[WQ.head].event_cycle <= current_core_cycle[writeback_cpu]) && (WQ.occupancy > 0)) {
        ...
        int way = check_hit(&WQ.entry[index]);
            ...
            while (now <= current_core_cycle[writeback_cpu])
            {
                ...
                if(cache_type == IS_L1D)
                {
                        //cout << "l1 hit cycle " << now << endl;
                        if(L1D_active_hit_cycles_per_core.find(writeback_cpu) == L1D_active_hit_cycles_per_core.end())
                        {
                                L1D_active_hit_cycles_per_core[writeback_cpu] = std::set<uint64_t>();
                        }
                        L1D_active_hit_cycles_per_core[writeback_cpu].insert(now);
                        //cout << L1D_active_hit_cycles_per_core[writeback_cpu].size() << endl;
                        if(L1D_active_hit_cycles_per_core[writeback_cpu].size() >= SET_MAX)
                        {
                                std::set<uint64_t>::iterator l1 = L1D_active_hit_cycles_per_core[writeback_cpu].begin();
                                L1D_active_hit_cycles_per_core[writeback_cpu].erase(l1);
                        }
                }
                else if (cache_type == IS_L2C)
                {
                        //cout << "l2 hit cycle " << now << endl;
                        if(L2C_active_hit_cycles_per_core.find(writeback_cpu) == L2C_active_hit_cycles_per_core.end())
                        {
                                L2C_active_hit_cycles_per_core[writeback_cpu] = std::set<uint64_t>();
                        }
                        L2C_active_hit_cycles_per_core[writeback_cpu].insert(now);
                        //cout << L2C_active_hit_cycles_per_core[writeback_cpu].size() << endl;
                        if(L2C_active_hit_cycles_per_core[writeback_cpu].size() >= SET_MAX)
                        {
                                std::set<uint64_t>::iterator l2 = L2C_active_hit_cycles_per_core[writeback_cpu].begin();
                                L2C_active_hit_cycles_per_core[writeback_cpu].erase(l2);
                        }
                }
 ...
 /*
		if(cache_type == IS_LLC)
		{
			cout << "L3 cache full_addr " << WQ.entry[index].full_addr << endl;
			cout << "L3 enqueued cycle " << WQ.entry[index].event_cycle << endl;	
		}
                else if (cache_type == IS_L1D)
                {
                                cout << "L1 cache full_addr " << WQ.entry[index].full_addr << endl;
                                cout << "L1 enqueued cycle " << WQ.entry[index].event_cycle << endl;
                }
                else if (cache_type == IS_L2C)
                {
                                cout << "L2 cache full_addr " << WQ.entry[index].full_addr << endl;
                                cout << "L2 enqueued cycle " << WQ.entry[index].event_cycle << endl;
                } 
*/
	    ...
	    //uint64_t start = now;
            ...
            int way = check_hit(&RQ.entry[index]);
                while (now <= current_core_cycle[read_cpu])
                {
                        ...
                        {
                                //cout << "l1 hit cycle " << now << endl;
                                if(L1D_active_hit_cycles_per_core.find(read_cpu) == L1D_active_hit_cycles_per_core.end())
                                {
                                        L1D_active_hit_cycles_per_core[read_cpu] = std::set<uint64_t>();
                                }
                                L1D_active_hit_cycles_per_core[read_cpu].insert(now);
                                //cout << L1D_active_hit_cycles_per_core[read_cpu].size() << endl;
                                if(L1D_active_hit_cycles_per_core[read_cpu].size() >= SET_MAX)
                                {
                                        std::set<uint64_t>::iterator l1 = L1D_active_hit_cycles_per_core[read_cpu].begin();
                                        L1D_active_hit_cycles_per_core[read_cpu].erase(l1);
                                }
                        }
                        else if (cache_type == IS_L2C)
                        {
                                //cout << "l2 hit cycle " << now << endl;
                                if(L2C_active_hit_cycles_per_core.find(read_cpu) == L2C_active_hit_cycles_per_core.end())
                                {
                                        L2C_active_hit_cycles_per_core[read_cpu] = std::set<uint64_t>();
                                }
                                L2C_active_hit_cycles_per_core[read_cpu].insert(now);
                                //cout << L2C_active_hit_cycles_per_core[read_cpu].size() << endl;
                                if(L2C_active_hit_cycles_per_core[read_cpu].size() >= SET_MAX)
                                {
                                        std::set<uint64_t>::iterator l2 = L2C_active_hit_cycles_per_core[read_cpu].begin();
                                        L2C_active_hit_cycles_per_core[read_cpu].erase(l2);
                                }
                        }
...
/*
		...
		if(cache_type == IS_LLC)
                {
                        cout << "L3 cache full_addr " << RQ.entry[index].full_addr << endl;
                        cout << "L3 enqueued cycle " << RQ.entry[index].event_cycle << endl;
                }
                else if (cache_type == IS_L1D)
                {
                                cout << "L1 cache full_addr " << RQ.entry[index].full_addr << endl;
                                cout << "L1 enqueued cycle " << RQ.entry[index].event_cycle << endl;
                }
                else if (cache_type == IS_L2C)
                {
                                cout << "L2 cache full_addr " << RQ.entry[index].full_addr << endl;
                                cout << "L2 enqueued cycle " << RQ.entry[index].event_cycle << endl;
                }
	    ...
	    uint64_t now = (PQ.entry[PQ.head].event_cycle - LATENCY >= 0)? PQ.entry[PQ.head].event_cycle - LATENCY : 0;
            ...
            int way = check_hit(&PQ.entry[index]);
                ...
                while (now <= current_core_cycle[prefetch_cpu])
                {
                        ...
                        if(cache_type == IS_L1D)
                        {
                                L1D_active_hit_cycles_per_core[prefetch_cpu].insert(now);
                                if(L1D_active_hit_cycles_per_core[prefetch_cpu].size() >= SET_MAX)
                                {
                                        std::set<uint64_t>::iterator l1 = L1D_active_hit_cycles_per_core[prefetch_cpu].begin();
                                        L1D_active_hit_cycles_per_core[prefetch_cpu].erase(l1);
                                }
                        }
                        else if (cache_type == IS_L2C)
                        {
                                L2C_active_hit_cycles_per_core[prefetch_cpu].insert(now);
                                if(L2C_active_hit_cycles_per_core[prefetch_cpu].size() >= SET_MAX)
                                {
                                        std::set<uint64_t>::iterator l2 = L2C_active_hit_cycles_per_core[prefetch_cpu].begin();
                                        L2C_active_hit_cycles_per_core[prefetch_cpu].erase(l2);
                                }
                        }
